...
int main(){
	...
	readE.open("Liquid/Data/BlockAveE.out");
	readK.open("Liquid/Data/BlockAveK.out");
	readU.open("Liquid/Data/BlockAveU.out");
	readT.open("Liquid/Data/BlockAveT.out");
	readP.open("Liquid/Data/BlockAveP.out");
	...
	readP.close();
	for(int i=0; i<2; i++) {
		writeE.open("Liquid/Elemento" +to_string(i+1) + "/BlockAveE.out");
		writeU.open("Liquid/Elemento" +to_string(i+1) + "/BlockAveU.out");
		writeK.open("Liquid/Elemento" +to_string(i+1) + "/BlockAveK.out");
		writeT.open("Liquid/Elemento" +to_string(i+1) + "/BlockAveT.out");	
		writeP.open("Liquid/Elemento" +to_string(i+1) + "/BlockAveP.out");
		for(int j=0; j<nblocks; j++) {
			...
			writeP<<j+1<<"   "<<P[j]*Tcar[i]*KB*(pow(10,27)/101325.)/pow(sigma[i],3)<<"   "<<stdevP[j]*Tcar[i]*KB*(pow(10,27)/101325.)/pow(sigma[i],3)<<endl;
		}
		writeE.close();
		writeU.close();
		writeK.close();
