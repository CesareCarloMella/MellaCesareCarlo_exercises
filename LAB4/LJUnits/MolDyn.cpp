...
using namespace std;
//costruttore **************
 MolDyn :: MolDyn(double T, int number, double dens, double taglio, double dtime, int npassi) {
	temp=T;				//temperatura
...
//IMPLEMENTAZIONE DELLO STATO POSIZIONI INIZIALE.
	ifstream ReadConf;
	...
	if(ReadConf.fail()==true) {		//...ma se non ci sono...
		...
		//ora le velocità sono compatibili con la temperatura.
  		...
  		if(abs((CheckTemp-temp)/temp*100.)<=10) {
  			cout<<"La temperatura è giusta entro il 10%, non correggo."<<endl;
  		} else {
  			fs = sqrt(3 * temp / sumv2);   // fs = velocity scale factor 
   			for (int i=0; i<n_part; ++i){
     				vx[i] *= fs;
