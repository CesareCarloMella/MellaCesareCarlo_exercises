...
int main()
{
	...
	//Leggo mu e sigma per il calcolo con funzione di partenza gaussiana ottimizzata.
	ifstream read("parametriFinali.out");
	read>>mu;
	read>>sigma;
	//fine lettura, per poter usare la funzione doppia gaussiana ottimizzata, scommentarla da sotto.
...
double variationalWaveFunction_second(double v)
{
	...
	//return ((v*v+mu*mu-var)/(var*var))*variationalWaveFunction(v)+(2*mu*v/(var*var))*(exp(-((v-mu)*(v-mu))/(2*sigma*sigma))-exp(-((v+mu)*(v+mu))/(2*sigma*sigma)));
}
