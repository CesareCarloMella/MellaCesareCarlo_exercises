	...
	//lettura dei parametri
        ...
        while(segnale==1) {
                ...
                for(int i=0; i<100; i++) {
                        ...
                        f.y=s.y+incr.y;
                        f.z=s.z+incr.z;
                        prob1=prob210(s);		//valuta le probabilità
                        prob2=prob210(f);		
...
//La delta e' ok. Procedo con la simulazione
        ofstream printR("risultati210/raggi210.out");
      	ofstream printP("risultati210/posizioni210.out");
        for(int i=0; i<N; i++) {
                prog[i]=0.;
                prog2[i]=0.;
        }
       ...
       printR.close();
       printP.close();
       printR.open("risultati210/progressivi210.out");
